...
type UsersAPI
   = "profile" :> AuthProtect "JWT" :> Get '[ JSON] UserProfile :<|> "register" :> ReqBody '[ JSON] Register :> Post '[ JSON] ReturnToken
...
usersServer connString =
  (fetchUserProfileHandler connString) :<|> (registerUserHandler connString)
...
loginHandler conn login = do
  maybeUser <- liftIO $ dbGetUserByLogin conn login
...
registerUserHandler connString Register {..} = do
  ...
  let hashedPassword = decodeUtf8 $ H.hash $ encodeUtf8 newUserPassword
  let newUser =
        User
