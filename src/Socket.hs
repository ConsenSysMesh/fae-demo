...
runSocketServer :: Secret -> Int -> ConnectionString -> IO ()
runSocketServer secretKey port dbConnString = do
  serverState <- newMVar initialServerState
  print $ "Socket server listening on " ++ (show port :: String)
  WS.runServer "127.0.0.1" port $ application secretKey dbConnString serverState
...
application :: Secret -> ConnectionString -> MVar ServerState -> WS.ServerApp
application secretKey dbConnString serverState pending = do
  newConn <- WS.acceptRequest pending
  WS.forkPingThread newConn 30
  msg <- WS.receiveData newConn
  authClient secretKey serverState dbConnString newConn $ Token msg
